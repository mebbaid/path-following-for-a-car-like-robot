...
for k = 1:50:size(x,1)
    ...
    thetasd1 = thetasd(k); 
    %xmr_delta1 = xmr_delta(k);  ymr_delta1 = ymr_delta(k); thetamr_delta1 = thetamr_delta(k);
    ...
    legend([c e sd mr],'Continuous-time','ZOH emulation', 'Approx Sampling', 'MR Sampling' ,'latex','FontSize',fontsize_labels);
    axis([-5 5 -5 5])
    pause(T/2)
    box on;
    grid on
